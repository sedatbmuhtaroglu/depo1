...
import { Plus, Minus, Coffee, QrCode, Monitor, ChevronRight, UtensilsCrossed } from "lucide-react";
...
export default function MenuClient({ data }: { data: MenuData }) {
  ...
  const canUseIyzicoForOrder = paymentMethods.iyzico;
  const hasAnyBillPaymentMethod =
    paymentMethods.cash || paymentMethods.creditCard || paymentMethods.iyzico;
  ...
  const linePrice = (item: CartItem) => {
    ...
    const extra =
      item.selectedOptions?.reduce((sum, g) => {
        const group = item.optionGroups?.find((og) => og.id === g.groupId);
        if (!group) return sum;
        const groupExtra = g.optionIds.reduce((inner, optId) => {
          const opt = group.options.find((o) => o.id === optId);
          return inner + (opt?.priceDelta ?? 0);
        }, 0);
  ...
  const getSelectedOptionsSummary = (item: CartItem) => {
    ...
    const labels = item.selectedOptions.flatMap((groupSel) => {
      const group = item.optionGroups?.find((g) => g.id === groupSel.groupId);
      if (!group) return [];
      return groupSel.optionIds
  ...
  const verifyOrderLocation = async (): Promise<CustomerLocation | null> => {
    ...
    if (
      ...
      setLocationStatusMessage(
        "Restoran konum ayarlari eksik. Lutfen isletme yoneticisine basvurun.",
      );
      return null;
    }
    ...
    if (!("geolocation" in navigator)) {
      ...
      setLocationStatusMessage(
        "Konum bilgisi alinamadi. Siparis icin konum dogrulamasi gerekli.",
      );
      return null;
    }
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          if (error.code === error.PERMISSION_DENIED) {
            ...
            setLocationStatusMessage(
              "Konum izni olmadan siparis verilemez. Lutfen izin verin.",
            );
          } else {
            setLocationStatus("unavailable");
            setLocationStatusMessage("Konum alinamadi. Lutfen tekrar deneyin.");
          }
          resolve(null);
        },
    ...
    if (distanceMeters > locationRadiusMeters) {
      ...
      setLocationStatusMessage(
        `Restorana yeterince yakin degilsiniz. Mesafe: ${Math.round(distanceMeters)} m, limit: ${locationRadiusMeters} m.`,
      );
      setVerifiedLocation(null);
      return null;
    }
    ...
    setLocationStatusMessage(
      `Konum dogrulandi (${Math.round(distanceMeters)} m / ${locationRadiusMeters} m).`,
    );
  ...
  const handleCreateOrder = async () => {
    ...
    if (orderingClosed) {
      toast.error("Su an siparis alinamiyor.");
      return;
    }
    if (!selectedPaymentMethod) {
      toast.error("Siparis oncesi odeme yontemi secmelisiniz.");
      return;
    }
    const verified = await verifyOrderLocation();
    if (requiresLocationEnforcement && !verified) {
      toast.error("Siparis verebilmek icin konum dogrulamasi gerekli.");
      return;
    }
    setIsSubmitting(true);
    const toastId = toast.loading("Siparisiniz olusturuluyor...");
    try {
      ...
      if (result?.success) {
        ...
        toast.error(result?.message || "Bir hata olustu.", { id: toastId });
      }
    } catch {
      toast.error("Siparis olusturulurken bir hata olustu.", { id: toastId });
    } finally {
  ...
  const handleRequestWaiter = async () => {
    ...
    try {
      ...
      toast.error("Garson çağırma isteği sırasında bir hata oluştu.", {
        id: toastId,
      });
    } finally {
  ...
  const handleOpenPaymentModal = () => {
    if (!canRequestBill) {
      toast.error("Bu masa icin odenecek acik hesap bulunmuyor.");
      return;
    }
    if (!hasAnyBillPaymentMethod) {
      toast.error("Su an aktif odeme yontemi yok.");
      return;
    }
  ...
  const handleRequestBillWithMethod = async (
    ...
    const toastId = toast.loading("Odeme islemi baslatiliyor...");
    try {
    ...
    <div className="storefront-menu min-h-screen pb-40 font-sans text-[#2d251d]" style={rootStyle}>
      ...
      {categories.length === 0 && products.length === 0 && (
        <div className="px-3 pt-3 sm:px-4">
          <div className="storefront-info mx-auto w-full max-w-2xl rounded-2xl px-4 py-3 text-center text-sm font-medium">
            Aktif bir menü bulunamadı. Lütfen işletme ile iletişime geçin.
          </div>
        </div>
      )}
      ...
      (frequentShowcase.placement === "BELOW_CATEGORIES" ||
        ...
        <div
          className={
            frequentShowcase.placement === "BLOCK"
              ? "mx-auto w-full max-w-2xl px-3 pt-2 sm:px-4"
              : "mx-auto w-full max-w-2xl px-3 pt-2 sm:px-4"
          }
        >
          <StorefrontFrequentShowcaseSection
            ...
            <div className="mt-auto border-t border-[#eadfce] p-4">
              ...
              <div className="mb-3 rounded-lg border border-[#eadfce] bg-[#fff8ed] p-3 text-xs text-[#6c5846]">
                ...
                <div className="mt-3 grid gap-2">
                  {canUseIyzicoForOrder && (
                    ...
                    >
                      ...
                      <p className="mt-0.5 text-[11px] text-[#8a7460]">
                        Online odeme, siparisten sonra odeme sayfasina yonlendirir.
                      </p>
                    </button>
                  )}
                  <button
                  ...
                  >
                    ...
                    <p className="mt-0.5 text-[11px] text-[#8a7460]">
                      Sipariş masa hesabınıza yazılır; ödeme hesap isterken seçilir.
                    </p>
                  </button>
                </div>
                {!canUseIyzicoForOrder && (
              ...
              >
                {isCheckingLocation
                  ...
                  : isSubmitting
                    ? t("Gonderiliyor...", "Submitting...")
                    : t("Siparisi Onayla", "Confirm Order")}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="storefront-sheet w-full max-w-sm rounded-t-3xl p-4 sm:rounded-2xl">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-white">
                {t("Odeme Yontemi Secin", "Choose Payment Method")}
              </h3>
              <button
                type="button"
            ...
            {!hasAnyBillPaymentMethod ? (
              <p className="text-sm text-amber-300">
                Bu restoran icin aktif odeme yontemi bulunmuyor.
              </p>
            ) : (
              <div className="space-y-2">
                ...
                {paymentMethods.creditCard && (
                  <button
                    ...
                    className="storefront-primary-btn w-full rounded-lg px-3 py-2 text-left text-sm font-semibold disabled:opacity-60"
                  >
                    {t("Kredi karti", "Credit Card")}
                  </button>
                )}
                {paymentMethods.iyzico && (
                  <button
                    type="button"
                    ...
                    className="storefront-primary-btn w-full rounded-lg px-3 py-2 text-left text-sm font-semibold disabled:opacity-60"
                  >
                    {t("IyziCo ile odeme", "Pay with IyziCo")}
                  </button>
                )}
              </div>
            )}
